...
export const loadRecipe = async function (id) {
  try {
    const modelData = await AJAX(`${API_URL}/${id}?key=${KEY}`);

    state.recipe = createRecipeObject(modelData);
...
export const loadSearchResults = async function (query) {
  try {
    ...
    const modelData = await AJAX(`${API_URL}?search=${query}&key=${KEY}`);
    state.search.results = modelData.data.recipes.map(rec => {
      return {
        id: rec.id,
        title: rec.title,
